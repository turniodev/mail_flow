...
import { 
    ...
    List, Braces, ChevronDown, CheckCircle2, Database
} from 'lucide-react';
...
const FormsTab: React.FC = () => {
  ...
  return (
    <div className="space-y-6 pb-40">
      ...
      >
          <div className="space-y-8 py-2">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <Input label="Tên định danh Form" placeholder="VD: Form Landing Page..." value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} autoFocus className="h-11" />
                  <div className="space-y-1.5">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Danh sách lưu trữ đích</label>
                      <Select options={lists.map(l => ({ value: l.id, label: l.name }))} value={formData.targetListId || ''} onChange={v => setFormData({...formData, targetListId: v})} placeholder="Chọn danh sách đích..." icon={Database} variant="outline" className="h-11" />
                  </div>
              </div>
                            ...
                            <div key={field.id} className="group relative flex flex-col sm:flex-row items-start sm:items-center gap-4 bg-white p-4 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md hover:border-blue-300 transition-all animate-in slide-in-from-bottom-2 duration-300">
                                ...
                                <div className="pl-4 w-full sm:w-56 shrink-0">
                                        ...
                                        <div className="relative">
                                            ...
                                            <select value={field.dbField} onChange={e => updateField(field.id, { dbField: e.target.value })} disabled={field.dbField === 'email'} className={`w-full pl-9 pr-8 py-2.5 rounded-xl text-xs font-bold appearance-none outline-none border-2 transition-all ${field.dbField === 'email' ? 'bg-slate-50 border-slate-100 text-slate-500' : 'bg-white border-slate-200 text-slate-700 hover:border-blue-300 focus:border-blue-500'}`}>
                                ...
                                <div className="flex items-center justify-between sm:justify-end gap-4 w-full sm:w-auto pt-2 sm:pt-6 border-t sm:border-t-0 border-slate-100 mt-2 sm:mt-0">
                                        <div onClick={() => field.dbField !== 'email' && updateField(field.id, { required: !field.required })} className={`flex items-center gap-2 cursor-pointer px-3 py-1.5 rounded-lg transition-all select-none ${field.dbField === 'email' ? 'opacity-50 pointer-events-none bg-slate-100' : 'hover:bg-slate-50'}`}>
                                            <div className={`w-9 h-5 rounded-full p-0.5 transition-all duration-300 flex items-center ${field.required ? 'bg-emerald-500 justify-end' : 'bg-slate-300 justify-start'}`}><div className="w-4 h-4 bg-white rounded-full shadow-sm"></div></div>
                                            <span className={`text-[9px] font-bold uppercase tracking-wider ${field.required ? 'text-emerald-600' : 'text-slate-400'}`}>Bắt buộc</span>
                                        </div>
                                        {field.dbField !== 'email' ? (<button onClick={() => setFormData({...formData, fields: formData.fields?.filter(f => f.id !== field.id)})} className="p-2 text-slate-300 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-all"><Trash2 className="w-4 h-4" /></button>) : (<div className="w-8"></div>)}
                                </div>
                            </div>
                          );
                      })}
                      <div className="p-4 bg-white/40 border-2 border-dashed border-slate-200 rounded-2xl text-center">
                          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wide flex items-center justify-center gap-2"><Sparkles className="w-3.5 h-3.5" /> Dữ liệu sẽ tự động đồng bộ vào hồ sơ khách hàng.</p>
                      </div>
                  </div>
              </div>
          </div>
      ...
      <ConfirmationModal isOpen={!!confirmDeleteId} onClose={() => setConfirmDeleteId(null)} onConfirm={executeDelete} title="Xác nhận xóa biểu mẫu" message="Hành động này sẽ xóa vĩnh viễn biểu mẫu này." variant="danger" confirmLabel="Xóa vĩnh viễn" />
